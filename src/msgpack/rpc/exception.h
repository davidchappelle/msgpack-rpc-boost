...
struct connect_error : rpc_error {
    connect_error() :
        rpc_error("connection failed") {}
...
struct remote_error : rpc_error {
    ...
    object error() const {
        return m_future.error();
    }

    template <typename T>
    T result_as() const {
        return m_future.result_as<T>();
    }

    template <typename T>
    T error_as() const {
        return m_future.error_as<T>();
    }

    auto_zone& zone() {
        return m_future.zone();
    }
